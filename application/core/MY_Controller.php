...
class Authenticate extends MY_Controller
{
	public function __construct()
	{
		...
		// Title
		$this->data['title'] = ( ! empty($this->config->item('dp_title'))) ? $this->config->item('dp_title') : 'Sembada Auth';
	}
...
class Backend extends MY_Controller
{
	public function __construct()
	{
		...
		else
		{
			...
			$this->data['lang_report']        	   = $this->lang->line('report');
...
class Staff extends MY_Controller
{
	public function __construct()
	{
		...
		{
			// Load model
			$this->load->model(array('Staff_tools_model'));

			// Load langage
			$this->lang->load(array('Staff'));

			// Theme
			$this->data['theme']     = ( ! empty($this->config->item('dp_theme_Staff'))) ? $this->config->item('dp_theme_Staff') : 'default';
			$this->data['theme_url'] = base_url($this->config->item('dp_theme_Staff_url'));
	...
	public function render()
	{
		...
		// Error management
		$this->data['result_flashdata'] = $this->Staff_tools_model->display_flashdata('item');

		// Include nav header
		$this->data['nav_header'] = $this->parser->parse('Staff/_theme/nav_header', $this->data, TRUE);

		// Include navside
		$this->data['nav_side'] = $this->parser->parse('Staff/_theme/nav_side', $this->data, TRUE);
		...
		// Render template
		$this->data['data'] = $this->data;
		$this->parser->parse('Staff/_theme/template', $this->data);
	}
}
